...
CLASS zcl_itab_nesting IMPLEMENTATION.
  ...
  LOOP AT artists INTO DATA(artist).
    DATA(nested_artist) = VALUE artist_album_nested_type(
    artist_id = artist-artist_id
    artist_name = artist-artist_name
    ).

    LOOP AT albums INTO DATA(album) 
      WHERE artist_id = artist-artist_id.
      
      DATA(nested_album) = VALUE album_song_nested_type(
      album_id = album-album_id
      album_name = album-album_name
      ).
      LOOP AT SONGS INTO DATA(song)
      ...
      ENDLOOP.
    APPEND nested_album TO nested_artist-albums.
    ENDLOOP.
    APPEND nested_artist TO nested_data.
  ENDLOOP.
ENDMETHOD.
